...
reportRouter.post('/general', authenticateJWT, async (req: Request, res: Response) => {
    ...
    try {
        const result = await pool.query(
            `INSERT INTO report (reporter_id, reason, description) VALUES ($1, $2, $3) RETURNING *`,
...
// Ad reports
reportRouter.post('/ad', authenticateJWT, async (req, res) => {
    const reporter_id = req.user!.id;
    const { ad_id, reported_user_id, reason, description } = req.body;

    if (!ad_id || !reported_user_id || !reason) {
        res.status(StatusCodes.BAD_REQUEST).json({ error: 'ad_id, reported_user_id and reason are required' });
        return;
    }

    try {
        const result = await pool.query(
            `INSERT INTO report (reporter_id, ad_id, reported_user_id, reason, description) VALUES ($1, $2, $3, $4, $5) RETURNING *`,
